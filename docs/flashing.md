...
Two firmware changes on top of the standard **Vial** firmware (see `firmware/totem/`): a tiny Raw
HID broadcast of the active layer, and a **half-duplex split link on `D7`/`GP1`** because `D6`'s
path is dead on this board. Vial keeps working.

Symptom that override fixes, for future reference: both halves type fine plugged in alone, but over
TRRS only the master works — in either direction. That is one dead data pin, not a firmware
mismatch. Flashing both halves with identical images does not help; moving the split to the
surviving pin does.
